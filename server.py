...
class PointOfInterestHandler(RethinkDBHandler):
    def get(self, *args):
        ...
        selection = r.table('osm_pois').get_nearest(
...
class OSMHandler(RethinkDBHandler):
    ...
    def get(self, *args):
        ...
        query_range = r.polygon(
            ...
            r.point(south, east),
            r.point(north, east))
        selection = (r.table('osm_nopoints')
        ...
        size = 0
        for chunk in chunked(cursor, 500):
            size += len(chunk)
            self.write_event(chunk)
        self.write_event('done')
        total_t = (datetime.now() - start_t).total_seconds()
        print 'OSM query took', initial_t, 's for the first batch',
        print '(', total_t, 's total) and provided', size, 'results.'


def main(port, host, rport):
    settings = {'host': host, 'port': rport}
    routes = [
        (r'/osm/(.*?)/(.*?)/(.*?)/(.*?)/?', OSMHandler),
...
if __name__ == '__main__':
    ...
    main(args.port, args.rhost, args.rport)
